<template>
    <div @mouseenter="status=true" 
        ...
        <div class="absolute top-[4rem] right-0 left-0 m-0 pb-[2rem] bg-[white] flex justify-center" 
        ...
        @mouseenter="status=true">
            <div class="flex gap-[1rem]">
                <div v-for="item in items" class="w-[26.25rem]">
                    ...
                    <div class="mt-[1.5rem] flex gap-[1.5rem] flex-col">
                        <div class="flex hover:bg-[#EDFBF7] p-[1rem] items-center w-auto rounded-[1rem] cursor-pointer" @click="onPage(item1?.path)" v-for="item1 in item.items">
                            <img  :src="item1.img" class="w-[1.5rem] h-[1.5rem] bg-[#01AA44]"/>
